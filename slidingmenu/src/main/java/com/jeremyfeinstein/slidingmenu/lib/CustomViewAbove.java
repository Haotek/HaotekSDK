...
public class CustomViewAbove extends ViewGroup {
	...
	private int getRightBound() {
		...
		{
			return mViewBehind.getSecondaryAbsRightBound(mSecondaryContent);
		}
		else
		{
	...
	protected float getSecondaryPercentOpen() {
		if(mSecondaryContent != null)
			return Math.abs(mScrollX-mSecondaryContent.getLeft()) / getSecondaryBehindWidth();
		else
			return 0;
	}
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		mViewBehind.drawFade(mContent, canvas, getPercentOpen(),getSecondaryPercentOpen());
